...
var Bignums = require(library);
var TWO_TO_POWER = 100;

var core = new Bignums.BIGNUMLIB(500);
...
function squared(n) {
  var core = new Bignums.BIGNUMLIB;
...
for (var x = 0; x <= TWO_TO_POWER; x++) {
  ...
  if (prev) {
    reveal(2,"**",x.toString(), core.multiply_bignum(TWO,prev));
  } else {
    reveal(2, '^', x.toString(), core.power_bignum(TWO, x));
  }
}
...
var core = new Bignums.BIGNUMLIB;
var a1 = new Bignums.BIGNUM;
var b2 = core.clone(a1);
a1.signbit = 2;
reveal(b2.signbit);
reveal(a1.signbit);
var c3 = core.clone(b2);
var c4 = new Bignums.BIGNUM;
core.clear(c4);
...
var EIK = new Bignums.BIGNUM();
EIK = core.toBignum(1);
var SIFR = new Bignums.BIGNUM();
reveal(SIFR, core.isZero(SIFR) ? "zero" : "not zero");
reveal(EIK, core.isZero(EIK) ? "zero" : "not zero");

var B = new Bignums.BIGNUM();
B = core.toBignum(1);
var x = core.subtract_bignum(core.ZERO(),B);
var y = core.subtract_bignum(B,core.ZERO());
reveal(x);
reveal(y); 